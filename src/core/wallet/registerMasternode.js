...
async function registerMasternode(
  ...
  const { result: proRegTxId } = await coreService.getRpcClient().protx(
    'register',
    collateralHash, // The txid of the 1000 xazab collateral funding transaction
    collateralIndex, // The output index of the 1000 xazab funding transaction
    ipAndPort, // Masternode IP address and port, in the format x.x.x.x:yyyy
    ownerAddress, // The new xazab address for the owner/voting address
    operatorPublicKey, // The Operator BLS public key
    ownerAddress, // The new xazab address, or the address of a delegate, used for proposal voting
    0, // The percentage of the block reward allocated to the operator as payment
    fundSourceAddress, // A new or existing xazab address to receive the owner’s masternode rewards
  );
